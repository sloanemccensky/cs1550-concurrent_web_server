...
int main()
{
	// for output stats file
	sema = sem_open("stats_proc.txt", O_CREAT, 0644, 1);
...
// function for process to initialize and begin serving
void handler(int connfd){
	...
	// logs end time and calculates total elapsed CPU time
	...
	// essentially mutex locking
	// prevents overlap writing of output file from different processes
	sem_wait(sema);
	FILE * stats_proc = fopen("stats_proc.txt", "a");
	fprintf(stats_proc, "%s, %d, %.4f\n", filename, file_stats.st_size, time_secs);
	fclose(stats_proc);
	sem_post(sema);
