...
// function that allows each thread to initialize and begin serving
void *handler(void *arg){

	// fprintf(stderr, "HANDLER TIME");

	// takes in start of elapsed CPU time
	...
	// take in end of elapsed CPU time
	...
	// don't want threads writing to the output file at the same time, so lock
	pthread_mutex_lock(&IO_stream);
	FILE * stats_thread = fopen("stats_thread.txt", "a");
	//fprintf(stderr, "hola");
	fprintf(stats_thread, "%s, %d, %.4f\n", filename, file_size, time_secs);
	fclose(stats_thread);
